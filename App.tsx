...
const TOOLS: Tool[] = rawTools.map(tool => ({
    ...tool,
    details: getToolDetails(tool.id, tool.title) // Inject SEO content here
}));
...
export default function App() {
  ...
  useEffect(() => {
    try {
        ...
        if (savedCat) setActiveCategory(savedCat as Category);
  ...
  }, [activeCategory, activeToolId, activePage]);
  ...
  const handleToolClick = (toolId: string) => {
      ...
      setDynamicRating(null); // Reset dynamic rating when switching tools
    ...
    if (navigator.share) {
        navigator.share({
            ...
            url: url
             ...
             // Fallback handled silently or if desired
        });
    } else {
  ...
  useEffect(() => {
      ...
      if (activeTool) {
          ...
          document.title = `${activePage.charAt(0).toUpperCase() + activePage.slice(1)} - MultiTools`;
      } else {
          document.title = "MultiTools - Bộ công cụ trực tuyến miễn phí: Đếm từ, Tính phần trăm, Mật khẩu";
      }
      ...
      if (activeTool) {
          ...
          // Determine Application Category based on Tool Category
            ...
            {activePage ? (
                ...
                <div className="animate-in fade-in zoom-in duration-300">
                    <div className="mb-6 flex items-start justify-between gap-4">
                        <div className="flex items-center gap-3">
                            <button 
                                onClick={() => { setActiveToolId(null); window.scrollTo({top:0, behavior:'smooth'}); }} 
                                ...
                                <div className="prose prose-slate max-w-none text-slate-600 bg-white p-6 rounded-2xl border border-slate-200">
                                    ...
                                    {!activeTool.details && (
                                        <>
                                            ...
                                            <p>
                                                Công cụ <strong>{activeTool.title}</strong> giúp bạn thực hiện các phép tính phức tạp chỉ trong vài giây. 
                                                Thay vì phải nhớ công thức hoặc tính nhẩm dễ sai sót, bạn chỉ cần nhập số liệu và nhận kết quả ngay lập tức.
                                            </p>
                                            <h3>Độ chính xác</h3>
                                            <p>
                                                Chúng tôi sử dụng các thuật toán chuẩn xác nhất để đảm bảo kết quả luôn đúng. 
                                                Dù bạn đang làm bài tập, tính toán tài chính hay quy đổi đơn vị, bạn hoàn toàn có thể tin tưởng vào kết quả này.
                                            </p>
                                        </>
                                    )}
